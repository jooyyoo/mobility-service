...
float Calculator::calculate_given_formula(
  const float & a,
  const float & b,
  const int8_t & operator1,
  const int8_t & operator2 = ArithmeticOperator::Request::UNKNOWN)
{
  float argument_result = 0.0;
  int8_t operator_to_use = operator1;
  if (operator_to_use == ArithmeticOperator::Request::UNKNOWN) {
    operator_to_use = operator2;
  }

  if (operator_to_use == ArithmeticOperator::Request::PLUS) {
    argument_result = a + b;
  } else if (operator_to_use == ArithmeticOperator::Request::MINUS) {
    argument_result = a - b;
  } else if (operator_to_use == ArithmeticOperator::Request::MULTIPLY) {
    argument_result = a * b;
  } else if (operator_to_use == ArithmeticOperator::Request::DIVISION) {
    argument_result = a / b;
    if (b == 0.0) {
